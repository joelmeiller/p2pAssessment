...
class TeammemberHandler extends DataHandler
{
	...
	public function __construct($id=null, $subhandler=null)
	{
	    ...
	    $this->getSql    = "SELECT id, username, name, ratingState, roleId FROM $this->table WHERE projectId=1";	
	    $this->getSqlWithId = "SELECT DISTINCT t.id, CASE WHEN id = $id THEN 'Eigenbewertung' ELSE t.name END AS 'name', ".
							  ...
							  "WHERE r.userId=";	
		$this->getByIdSql= "SELECT id, username, name, ratingState, roleId FROM $this->table WHERE id=";
	...
	public function getItemsByProject($includeRating=true)
	{
		...
		foreach( $list as $item ){
			...
			if( $includeRating ){
				$teammemberHandler = new TeammemberHandler($item['id'], new TeammemberRatingHandler( $item['id'] ));
