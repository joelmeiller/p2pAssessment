...
class User extends TeammemberHandler
{
	...
	{		
	    ...
	    try
	    {
			...
			if( $userItem = $userResult->fetch_assoc()){
				...
				$this->supervisor = ($userItem['isAdmin'] == '2');
			}
		}
		catch(Exception $e)
	    {
	...
	public function toJson(){
		...
		$optionItem = array();
		$optionItem['self'] 	= (!($this->admin || $this->supervisor) && !$this->openRating);
		$optionItem['all']  	= $this->admin || $this->supervisor;
		
		//Prepare rating options
		$ratingItem = array();
		$ratingItem['available'] = $this->ratingAvailable && !($this->admin || $this->supervisor);
